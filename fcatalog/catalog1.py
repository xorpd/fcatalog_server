...
def sign(my_str,num_perms):
    ...
    assert len(my_str) >= nbytes

    res_sign = []

    for p in range(num_perms):
        num_iters = len(my_str) - nbytes + 1
        cur_sign = min([perm(p,str_to_num(my_str[i:i+nbytes])) for i in \
                range(num_iters)])
